 ...
 */
public class InfiniDBExportMapper<KEYIN, VALIN>
extends SqoopMapper<KEYIN, VALIN, NullWritable, NullWritable> {
  ...
  /**
   * Create a named FIFO, and start cpimport connected to that FIFO.
  ...
  private void initCpImportProcess() throws IOException {
    ...
    int outputFieldDelim = conf.getInt(MySQLUtils.OUTPUT_FIELD_DELIM_KEY, (int) '|');
    ...
    if (0 != outputFieldDelim) {
        LOG.info("outputFieldDelim=" + outputFieldDelim);
        args.add("-s " + (char)outputFieldDelim);
    }

    if (0 != enclosedBy) {
        args.add("-E " + (char)enclosedBy);
    }
    if (0 != escapedBy) {
        args.add("-C " + (char)escapedBy);
    }
    ...
    cpImportProcess = Runtime.getRuntime().exec(args.toArray(new String[0]));
